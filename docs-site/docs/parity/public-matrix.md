...
**用户指南级叙事映射（How to use）**：[工程记忆 §14](/concept/engineering-memory-quantinuum)（How-to 映射）对照官方 [How to use InQuanto](https://docs.quantinuum.com/inquanto/manual/howto.html) 与本仓库模块 / `repro` 出口。

**逐项闭合执行计划（B→J 顺序，L1 对拍定义）**：[与InQuanto能力差距与实施计划 — 附录 D](/parity/gap-implementation-plan#appendix-d)

**差距清单 + 分阶段计划（维护稿）**：[与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan)（机读分类：`qchem_stack.protocols.inquanto_contract.inquanto_gap_categories`）。**L1 签字清单**：[附录 C](/parity/gap-implementation-plan#appendix-c)。

**边界（必读）**：把「公开叙事 + 可检证」做到底是 **L1**；**完全复现 InQuanto 闭源包在内部默认值、全部 driver 组合与商业运行时细节**属于 **L0**，在无源码与许可下**不可作为工程承诺**。见 [工程记忆 §0](/concept/engineering-memory-quantinuum)。

**图例**：`yes` 已实现 / `partial` 部分或 API 形态不同 / `no` 未实现 / `n/a` 刻意不做或非公开范围

## 0. 非云、非专有硬件：对齐范围与「超越」（L1+）

**全面对齐（本仓承诺）**：除下述 **刻意排除** 项外，矩阵 §1–4 与机读表 `inquanto_gap_categories()`、`GET /v1/meta/capability-surface` **同源**；`partial` 均配有 gap 锚点、caveat 或 [Y1 SLA 模板（附录 B §6）](/parity/gap-implementation-plan#y1-residual-partial-sla-template)。
...
| `AlgorithmAdaptVQE` / FermionicAdapt | `partial(interface-ready)`：`adapt.py` 已切到 **commutator gradient** + 可执行 pool registry（`fermionic_uccsd`/`toy_pair_xx`）；新增 `quantum.algorithm: tetris_adapt`（实验态） |
| `AlgorithmIQEB` | `partial(interface-ready)`：`IQEBVQE` 支持 `iqeb_n_grads` + `iqeb_energy_tolerance` + pool registry（默认 `iqeb_qubit_excitation`） |
...
| `Algorithm*QPE` | `partial(interface-ready)`：新增 `quantum/algorithms/qpe.py`（Deterministic/Kitaev/InfoTheory）并接入 demo track；主配置树仍通过 `qpe_demo_track_after_variational` / `qpe_pipeline_integration`；`run_summary.qpe_open_stack_contract_v1` |
| `AlgorithmVQS` / `AlgorithmMcLachlan*` | `partial(interface-ready)`：`quantum/algorithms/vqs.py` + **YAML 侧车** `quantum.vqs_track_after_variational` \| `quantum.vqs_pipeline_integration` → `vqs_track`，示例 `configs/example_h2_vqs_track.yaml`；RHS 仍占位 |
| `AlgorithmBayesianQPE` + Phayes | `partial`：`qpe_qec_demo/bayesian_stub.py`（`BayesianQPEStub`）；模块说明见仓库根下 `src/qchem_stack/qpe_qec_demo/README.md`（本站不内嵌该路径，请在克隆根打开）；并入 `qpe_demo_track` / `run_summary.qpe_demo_track_ran`；单测 `tests/test_l1_phase_c_iqeb_bayesian.py` |
| YAML `quantum.algorithm_factory` / `variational_plugins` | `partial`：示例 `configs/example_h2_echo_variational_plugin.yaml`；parity 导出 **`variational_registry_export_v1`**（`algorithm_registry_alignment_v1`，`resource_estimation_preview` 开关） |
| `quantum.adapt_pool_id` / `quantum.iqeb_pool_id` | `partial`：`operator_pool_registry.py` + **`operator_pool_registry_export_v1`**（**`pool_id_aliases`**：`qubit_excitation`、`uccsd_jw`）；示例 `configs/example_h2_adapt_singles_pool.yaml`、`example_h2_adapt_uccsd_jw_alias.yaml`、`example_h2_iqeb_qubit_excitation_alias.yaml` 等；`run_summary` 池 id；**`GET /v1/meta/capability-surface`** 内含同块导出；gap **`adapt_iqeb_operator_pool_surface`** |

**Registry 钉扎（P2-W5）**：YAML `quantum.algorithm`、`quantum.algorithm_factory`、算符池、变分 ansatz、fermion→qubit 映射的机读对照表见 [附录 A §11](/parity/gap-implementation-plan#p2-w5-algorithm-registry-alignment)（与上表各行交叉维护）。

## 3. Classical chemistry & embedding

| 能力 | qchem_stack | 备注 |
|------|-------------|------|
| PySCF RHF / active space → qubits | `yes`：`chem/drivers`, `hamiltonian.py`；`active_space.strategy=cas|manual|avas_stub|avas`（**`avas_stub`**：`configs/example_h2_avas_stub.yaml`；**`avas`**：`configs/example_h2_avas.yaml`，PySCF **`mcscf.avas.AVAS`** + `qchem_active_space_resolution_v1`；**CASSCF**：`casscf_orbital_optimization_audit` / **`casscf_orbital_optimization_for_integrals`** 共用单次 kernel）；**几何**：`molecule.ecp`、`molecule.zmatrix`（与 Cartesian 互斥）；**RI/DF**：`scf.density_fit`/`density_fit_auxbasis`（`driver_meta` 可追溯）；冻轨：`active_space.frozen_orbitals` → CASCI **`frozen`**；**轨道钩子**：`chemistry_extended.mo_coeff_transform_hook`；**一电子算符 API**：`PySCFDriver.compute_one_electron_operator_*`；restricted MO 「问题三元组」路径仍要求 **闭壳层 RHF**。**InQuanto 产品级** AVAS/CASSCF 编排仍为 **`partial`**，边界 [附录 A §10](/parity/gap-implementation-plan#p2-w3-avas-casscf-boundary)） |
...
本仓库 **单仓克隆** 常不含 `PandM/` 长篇文献树。若你持有完整 monorepo，可在 **`PandM/materials/learning/quantum-chem/literature/`** 下检索「Quantinuum 竞品研究」「复现线」「数据流与判据」等文件名。  
**本包竞争定位与分阶段目标**（相对 Quantinuum 产品路线）：见本站 [竞争定位与路线图](/concept/competitive-positioning) 或仓库母稿 `docs/竞争定位与路线图_对标Quantinuum产品与技术路线.md`。
