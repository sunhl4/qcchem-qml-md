...
- `repro`：`embedding_config` 全量嵌入块 + `parity_snapshot` 扁平字段；**本文**与 [parity 矩阵](/parity/public-matrix) 同步。
...
详见 [不排期项](/parity/backlog-to-schedule)（**仍非** 商业云与闭源同构；仅本地可审计类比）。
...
| 日段 | 交付物 | 验收标准（DoD） |
|------|--------|----------------|
| D1–D2 | **矩阵与机读表一致** | [parity 矩阵](/parity/public-matrix) 与 `inquanto_contract.inquanto_gap_categories()`、`INQUANTO_TO_QCHEM_OBJECT_MAP` **两两无矛盾**；任一行 `partial` 须在 gap 或矩阵备注中指明**缺失键或行为**。 |
...
| D3–D4 | **编译 / TKET 公开叙事** | [pytket 桥接](/reference/circuitir-tket-jobs) 与 `parity_integrations.tket_first_circuit_stats` 行为一致；CI 中 pytket 路径 **保持绿**；矩阵「编译」行备注与 `compiler_pass_bundle` gap **一致**。 |
...
| 日段 | 交付物 | 验收标准（DoD） |
|------|--------|----------------|
| D1–D3 | **Algorithms 行级对齐** | 矩阵 §2 每一 `Algorithm*`：`repro.run_summary` 或算法块内具备 **可写入论文** 的 `algorithm`/`meta` 字段集；VQD **三通道**、QSE **shot 模式**、SCEOM **矩阵元噪声语义** 与 [工程记忆](/concept/engineering-memory-quantinuum) 一致；**缺项**仅允许以 `caveat` 固定文案出现。 |
...
| D4–D5 | **QPE / Bayesian stub 双轨** | `qpe_demo_track` + `configs/` 样例与 [竞争定位 §5 P2](/concept/competitive-positioning) 一致；`run_summary` 中 **可检索** QPE 轨是否跑过。 |
