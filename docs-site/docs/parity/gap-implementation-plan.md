...
**目的**：在 [公开 parity 矩阵](/parity/public-matrix) 与 [竞争定位](/concept/competitive-positioning) 之外，给出一份**可维护**的「差什么 → 怎么补」清单，并注明**不追求**与闭源或商业云 1:1。实现层机读入口：`qchem_stack.protocols.inquanto_contract`（`classify_pauli_expectation_path`、`inquanto_gap_categories`）。
...
| **经典化学** | 多 driver（COSMO、PBC、k 点、CASSCF/AVAS 等） | **现阶段端到端数值以 PySCF 收口**（`scf.driver=pyscf`）；统一 interchange（`create_solver`、`ClassicalMeanFieldReference`、`SolverCapabilities`、`mean_field_meta` / classical benchmarks registry —— [竞争定位 §5.1](/concept/competitive-positioning)）；**ddCOSMO**；**PBC**（RHF@Γ 或 **KRHF**+`pbc_kpoint_mesh`；CASCI 用 `pbc_active_space_kpoint_index`；样例 `configs/example_h2_pbc_gamma.yaml`）；**多原子 CAS(4,4)**：`configs/example_h2o_sto3g_cas44.yaml`、`configs/example_n2_sto3g_cas44.yaml`（仍为 CASCI 型主路径）；**嵌入**：Schmidt 生产 + **`schmidt_dmet_max_cycles>1`** 时 **Schmidt–DMET 密度反馈**（`integrations/schmidt_dmet_self_consistent`）+ 通用 **`DMETSelfConsistencyLoop`**（多轮 bath 仍靠用户钩子）+ **`schmidt_bath_sidecar_json_path`**（用户 JSON→`embedding_workflow`）+ **`oniom_layers_v1`** 玩具层（`configs/example_oniom_toy.yaml`）+ `whole_active_system` / DMET / plugin；**projection** L1 轨迹（`configs/example_h2_projection_trace.yaml` 等）；**PySCF 活性空间**：`active_space.strategy=avas` + `chemistry_extended.avas_*`（阈值投影 → `qchem_active_space_resolution_v1`，`configs/example_h2_avas.yaml`）；可选 **`casscf_orbital_optimization_for_integrals`** / **`casscf_orbital_optimization_audit`**（共用单次 `mcscf.CASSCF` kernel，`configs/example_h2_casscf_audit.yaml`）；Psi4 等为 scaffold | **partial**：**InQuanto 级默认 AVAS/CASSCF 产品全流程**相对开放栈仍 **partial**；PySCF **阈值 AVAS** 与 **CASSCF→活性积分**已接线可检证；Psi4 **无** AVAS；PBC+溶剂受 PySCF 版本约束；闭源级 DMET bath **算法** L0 不宣称；其它 quantum chemistry **数值**深度仍以 PySCF 为先 |
...
| **协议 run** | 云侧 shot + DataFrame 一体 | 五阶段 + `protocol_counts` + 三能量路径 | **已较强**：现含 **精确 / statevector 采样 / Qiskit 比特串**（见 [技术文档_设备比特串与Qiskit采样路径.md](/reference/qiskit-shot-counts)） |
...
- `repro.parity_snapshot` 与 `repro.run_summary` 增加：  
  ...
  - **HTTP**：`GET /v1/meta/capability-surface` 含 **`open_stack_differentiators`**（`open_stack_differentiators_v1`），钉扎*非云、非专有硬件*下相对公开文档包的可检证长板（与 [parity 矩阵 §0](/parity/public-matrix) 一致）。  
  - Active-space：`active_space.strategy=cas|manual|avas_stub|avas`（**`strategy=avas`** 仅 `scf.driver=pyscf`，需非空 **`chemistry_extended.avas_ao_labels`**）。**`strategy=avas`**：`chem.active_space.pyscf_active_space_hooks`，`driver_meta` 键 **`qchem_active_space_resolution_v1`**，回填 YAML 活性尺寸；透传 **`hamiltonian_meta.pyscf_driver`**。  
  - 可选经典 benchmark：`chemistry_extended.classical_benchmark_enabled: true` 时附加 `classical_benchmarks`（**`qchem_classical_post_hf_benchmarks_v1`**；**`classical_benchmark_backend`**：`auto|stub|pyscf|psi4`）与 `run_summary` 镜像键。  
...
以下批次**不再在本节重复证据链**；维护者只更新 **§1 差距表**、**§2 机读键**、[inquanto_public_parity_matrix.md](/parity/public-matrix) 与本表**状态**，避免与 [竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning)、[工程记忆_Quantinuum对标与数据流技术文档.md](/concept/engineering-memory-quantinuum) 三处重复长清单。
...
**进行中**：路线图 **P2**（§141 残余、WBS、闸门）→ **附录 A**；**Y1 台账 / L3 / 年度残余 SLA** → **附录 B**（锚点同上）。
...
- 新增长板或关闭差距时：更新 [inquanto_public_parity_matrix.md](/parity/public-matrix) 相应行、本文 §1、以及 `inquanto_contract.inquanto_gap_categories()`。  
...
- **InQuanto 手册 How-to**：与公开 [How to use](https://docs.quantinuum.com/inquanto/manual/howto.html) 的模块级对齐见 [工程记忆 §14](/concept/engineering-memory-quantinuum)。

---

## 6. 路线图 §141 残余项（P2，禁止冒充 P1 `yes`）

与 [竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning) §141「仍需推进」一致；矩阵未升格前不关闭为 `yes`。**执行细化（WBS、闸门、非目标）** 以 **附录 A** 为准。

### P2-W3：AVAS / 产品 CASSCF 与「最小审计」分界

- **差距表 §1「经典化学」行**仍标 **`partial`**：**InQuanto 级全套「产品默认」AVAS/CASSCF UX**不等于已交付；阈值 **AVAS（PySCF）**与 **单次 CASSCF→积分（可选 audit）**已机读闭环。  
- **已有机读**：`casscf_orbital_optimization_audit`、`casscf_orbital_optimization_for_integrals`（见 `configs/example_h2_casscf_audit.yaml`）；AVAS：`configs/example_h2_avas.yaml` + `parity_snapshot`/export。  
- **设计母页**：**附录 A** 内锚点 [`#p2-w3-avas-casscf-boundary`](/parity/gap-implementation-plan#p2-w3-avas-casscf-boundary)（与矩阵 §3 同步维护）。

| 阶段（示意） | 主题 | 交付物方向 |
|--------------|------|------------|
| P2 初波 | QPE/FT × 资源 × 编译 | `run_summary` / `protocol_counts` 与 TKET 探针联合叙事；超越 demo 的资源估计可选 |
| P2 初波 | 分解与大体系 | **已接管线**：Schmidt–DMET **密度反馈**（`schmidt_dmet_max_cycles>1` + `schmidt_dmet_self_consistent`）与通用 `DMETSelfConsistencyLoop` 钩子；P2 仍推进 bath/ONIOM/QM-MM **化学意义上最小可跑** demo |
...
**文档角色**：在 [竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning) §6、§141 残余与 [与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan) 之上，给出 **P2 阶段** 的可执行分解（WBS、里程碑、闸门、非目标）。  
**术语**：本文 **路线图 P2** = 竞品文档中的「研究深度与大体系」阶段；**不等于**「主线结构增强」历史批次（QPE 演示轨接入、Computable 薄层、TKET CI 等——已交付并收进 [与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan) **§3 摘要表**）。

---

### 1. 与 P1 的边界

**P1（广义已闭合）**：L1 公开契约下，`repro` / `parity_snapshot` / export / CI 与矩阵 **`n/a` 诚实降级**（含 TN、BK/SCBK UCCSD Trotter）对齐；UCCSD Trotter（JW）、ZNE 机读合一、Schmidt bath 侧车、ONIOM 玩具层、CASSCF 审计轨道一步、教程与双 parity 等已落地（见竞争定位 §6「已闭合批次」与 [附录 E](/parity/gap-implementation-plan#appendix-e)）。
...
| **P2-W4** | 缓解进阶块 | 可选 DAG 节点或协议阶段；`parity_snapshot` 键 | 与 [mitigation_PMSV_ZNE_Qermit_mapping.md](/concept/mitigation-mapping) 映射一节同步 |
...
| **M-P2-d** | Q4 | W7 打包；残余 `partial` 填入 [附录 B §6](/parity/gap-implementation-plan#y1-residual-partial-sla-template) 或升级 Y3 项 |

（若与 **Y1 台账** Q3/Q4 重叠，以台账 [附录 B](/parity/gap-implementation-plan#appendix-b) 季度 OKR 为准，本文 WBS 作二级拆分。）
...
3. **[inquanto_public_parity_matrix.md](/parity/public-matrix)** 与 **`inquanto_gap_categories`** 无矛盾；禁止未文档化的 `parity_snapshot` 顶键。  
...
以下按 **依赖少 → 可并行 → 风险可控** 排序；与 [附录 E](/parity/gap-implementation-plan#appendix-e) §6 衔接。每步结束仍走 **§5 出口闸门**（pytest 全量、`check_parity_export_sample`、矩阵 ↔ gap、双站链接）。
...
- **文档总索引**（母稿全表、合并记录、阅读顺序）：[README.md](/tutorial/quickstart) 与 [CONTRIBUTING.md](../../../CONTRIBUTING.md)。  
- 战略总表：[竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning) §5–§6。  
- 差距总表与维护约定：[与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan)。  
- **文档站镜像（VitePress）**：中文全文路由 `/concept/p2-detailed-plan`，英文摘要 `/en/concept/p2-detailed-plan`（`npm run docs:dev` / `docs:build` 生成站点内路径）。  
- **P2-W5（registry）**、**ADR（P2-W2 分解范围）**、**P2-W3（经典边界）**：见本节 **§9–§11**（[§9 ADR](#adr-p2-w2-decomposition-scope) · [§10 W3](#p2-w3-avas-casscf-boundary) · [§11 W5](#p2-w5-algorithm-registry-alignment)；原独立 `ADR_P2_decomposition_scope.md`、`P2_W3_classical_avas_casscf_boundary.md`、`P2_W5_algorithm_registry_alignment.md` 已并入，技术表与决策 **全文保留**）。**MD 冻结**：[工程记忆 §16](/concept/engineering-memory-quantinuum)。  
...
|------|---------------------------|------|------------|
| **1–2** | `YYYY-MM-DD` ~ `YYYY-MM-DD` | 并行打底 | 本节 **[§11](#p2-w5-algorithm-registry-alignment)**（registry）、**[§9](#adr-p2-w2-decomposition-scope)**（ADR）；[工程记忆 §16](/concept/engineering-memory-quantinuum)；文档站 [新用户三条路径](/guide/onboarding-three-paths) |
...
| **5–6** | `YYYY-MM-DD` ~ `YYYY-MM-DD` | P2-W3 + W4 | 差距表 / 矩阵 §3 AVAS–CASSCF 边界（本节 **[§10](#p2-w3-avas-casscf-boundary)**）；[mitigation_PMSV_ZNE_Qermit_mapping.md](/concept/mitigation-mapping) §「P2 进阶块（双月）」 |
...
**关联母稿**：§6 序 1–9 与上表逐行对应；[附录 E](/parity/gap-implementation-plan#appendix-e) §6 声明 P1 已闭合后从 **本表第 1 行** 开工。
...
- 差距表与 [inquanto_public_parity_matrix.md](/parity/public-matrix) §3 **DMET / embedding** 行保持 **`partial`**，本 ADR 作为 caveat 引用。  
- 若产品方强制轨 B，需 **新开 ADR** 修订优先级并更新 [附录 B §6](/parity/gap-implementation-plan#y1-residual-partial-sla-template) 对应行季度。

### 9.4 链接（ADR）

- [与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan) §1 经典化学 / Ansatz。  
...
- 矩阵 [inquanto_public_parity_matrix.md](/parity/public-matrix) §3「PySCF / active space」行与本节一致时即视为文档闭合。  
...
| `adapt` | `adapt.FermionicAdaptVQE` | ADAPT-VQE；pool 与日程见 [inquanto_public_parity_matrix.md](/parity/public-matrix) §2 |
...
- 新增 registry 键：同步本表 + [inquanto_public_parity_matrix.md](/parity/public-matrix) §2（若影响对外叙事）+ `export_parity_criteria_table` 稳定列（若导出暴露名称）。  
- P2 双月闸门见上文 **§5**。
...
| 本次台账起始钉扎 | 2026-04-28（与 [附录 C](/parity/gap-implementation-plan#appendix-c) 一致） |
...
| **非云「超越」机读钉扎** | `GET /v1/meta/capability-surface` → **`open_stack_differentiators`**（`open_stack_differentiators_v1`）；矩阵 [§0](/parity/public-matrix) 与 [竞争定位 §3 卖点 6](/concept/competitive-positioning) |
| **P2 双月周历（8 周）** | 排期与周交付物见 [附录 A](/parity/gap-implementation-plan#appendix-a) **§8**；与 §6 执行序同源，季度 OKR 可引用 §8 行作为子任务 |
...
|------|------------------|---------|-----|----------------------------------|------|
| Y1-M01 | | | | | |
| Y1-M04 | **3** | **14** | **2**（含 `tensornet`→矩阵/`gap` **n/a**） | 0 | **广义 P1 排期闭合**；主表 §1–§3 行计数见 `python scripts/count_parity_matrix_main_tables.py`（示例输出：`yes=3 partial=14 n/a=2`，与手填冲突时以 gap 语义为准）；后续路线图见 [附录 A](/parity/gap-implementation-plan#appendix-a) |

*说明：行数统计以 [inquanto_public_parity_matrix.md](/parity/public-matrix) 主表 §1–§3 为准；脚本辅助计数，主观分类仍以 `inquanto_gap_categories` 与矩阵备注为准。*

---

### 3.5 节点级 backlog（295 manifest 节点）

**与矩阵的关系**：parity 矩阵按 **能力 / 模块行** 收敛；[`inquanto-node-backlog.generated.json`](../../../docs/inquanto-node-backlog.generated.json) 按 **InQuanto 公开 IA 节点（manifest 叶 + 类叶）** 展开，字段含附录 C 同构的 **验收项、平台维度、`differentiator_focus`**。二者通过 `qchem_module` 与 `parity_doc_hint` 弱关联。
...
**人读索引表**：[inquanto-node-backlog.generated.md](../../../docs/inquanto-node-backlog.generated.md)。**深度拆解**仍读架构报告 [appendix-C-deep-node-architecture.generated.md](../../../docs/architecture-report-quantinuum-inquanto-web/appendix-C-deep-node-architecture.generated.md)。
...
- **295 节点机读 backlog**：[`inquanto-node-backlog.generated.json`](../../../docs/inquanto-node-backlog.generated.json) · [Schema](../../../docs/inquanto-node-backlog.schema.json)
- **InQuanto B→J 逐项闭合计划（L1 序列表 + DoD）**：[附录 D](/parity/gap-implementation-plan#appendix-d)
- **公开手册 How-to（功能参考）**：[工程记忆 §14](/concept/engineering-memory-quantinuum)（锚 [How to use InQuanto](https://docs.quantinuum.com/inquanto/manual/howto.html)）
- 架构边界：[工程记忆 §0](/concept/engineering-memory-quantinuum)
- 竞争策略：[竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning)
- 签字清单：[附录 C](/parity/gap-implementation-plan#appendix-c)；维护角色：[CONTRIBUTING.md](../../../CONTRIBUTING.md)
- **P1 全量核对报告**：[附录 E](/parity/gap-implementation-plan#appendix-e)
...
| **矩阵 §2 `Algorithm*QPE`** | 演示轨 + Methods 合一（浅层） | partial；**深度资源估计** → [附录 A](/parity/gap-implementation-plan#appendix-a) P2-W1 | Y2-Q1 | 见 CONTRIBUTING | `example_h2_qpe_track.yaml`、`example_h2_qpe_track_parity_integrations.yaml`（pytket） |
...
**实施索引**（P1 全量核对后）：队列来源见 [附录 E](/parity/gap-implementation-plan#appendix-e) §5。
...
**契约**：与 [`docs/inquanto_public_parity_matrix.md`](/parity/public-matrix) + `qchem_stack.protocols.inquanto_contract` **同源**；**排除**真 Nexus/HQC 云产品与 H 系硬件标定。
...
| §1 Qermit | `qermit_graph` | `mitigation/qermit_analog`，`mitigation_execution_model_public`，`parity_snapshot.zne_qiskit_unification_v1`（ZNE×Qiskit Pauli），见 [mitigation_PMSV_ZNE_Qermit_mapping.md](/concept/mitigation-mapping) | 非闭源 Qermit |
...
| ID | 证据 | caveat |
|----|------|--------|
| **H1**（激发态 C10–C14） | CI：`pytest -m l1_excited`；代表测 `tests/test_qse_sceom_vqd_extended.py`（`l1_excited`）、`tests/test_orchestration_pipeline.py`；残余 `partial` 见 [附录 B §6](/parity/gap-implementation-plan#y1-residual-partial-sla-template) | 与闭源算法深度非 L0 |
| **I1**（MD/ML） | CI：`pytest -m l1_md_ml`；`md_bridge/`，`tests/test_md_bridge.py`；竞争定位 [竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning) | 差异化长板，非 InQuanto 主线 |

**公开文档钉扎（Quantinuum InQuanto）**

- 对照站点：`https://docs.quantinuum.com/inquanto/`（含 protocols、algorithms、errmit、cutensornet 等手册/API）。
- **How-to 工作流总览**：官方 [How to use InQuanto](https://docs.quantinuum.com/inquanto/manual/howto.html) 与本栈模块映射见 [工程记忆 §14](/concept/engineering-memory-quantinuum)（章节改版时与矩阵一并 diff）。
- **最近一次人工锚定日期**：2026-04-30。若公开站改版，按 [与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan) §5 做一次矩阵 + `inquanto_gap_categories` 差异登记（不自动视为功能回退）。
...
**workflow-preview ↔ repro（P1）**：默认 `repro.workflow_preview_v1` 与 `POST /v1/meta/workflow-preview` 在 **同 YAML、不含** `computables_rich` 时同源（`tests/test_workflow_preview_repro_alignment.py`）。若 Methods 需要 rich 块入 repro，置 `parity_integrations.include_computables_rich_in_repro: true`（与 API `include_computables_rich` 对齐）。**异步作业**：`GET …/summary` 在 `DONE` 前为 `partial`；与 Nexus 云 UX 非 1:1 的键边界见 [技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](/reference/http-api-sqlite-jobs)。

**广义 P1 计划闭合纪要（对照计划判据 1–5）**：§141 五项已在 [竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning) 「仍需推进」段与矩阵/`inquanto_gap_categories`中落实为 **已交付 YAML + pytest/export** 或 **`n/a`+原因**（含 TN、`BK/SCBK` UCCSD Trotter）；差距总表 §1 已与源码及导出一致；双 parity 矩阵与 `check_parity_export_sample.py` 抽样覆盖新增 YAML；B→J 逐项闭合见 [附录 D](/parity/gap-implementation-plan#appendix-d)。
...
**权威差距叙述**：[inquanto_public_parity_matrix.md](/parity/public-matrix)、[与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan)。**机读 gap**：`qchem_stack.protocols.inquanto_contract.inquanto_gap_categories`。
...
|----|----------|------------------|---------|--------|-------------|
| **1** | B1 五阶段 Protocol | 否 | 五阶段语义、三能量路径、异步边界文档与机读一致 | 矩阵 B1 行 caveat 收束；`protocol_counts` / `repro` 时间线字段（若需）与 [技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](/reference/http-api-sqlite-jobs) 一致 | `pytest` 覆盖五阶段 + 至少一条 Qiskit shots 路径 smoke；export 样例无未文档化键 |
| **2** | B2 `dataframe_circuit_shot` | — | **签认 yes** | 矩阵明示 yes；资源行与 [技术文档_CircuitIR与TKET桥接及作业契约.md](/reference/circuitir-tket-jobs) § 一致 | 现有资源单测 + export `excited_resource` 不回归 |
...
| **4** | B4 HTTP 作业网关 | 否（非真 Nexus） | **本地类比**能力与限制写死 | [launch_retrieve_nexus_analog.md](/concept/launch-retrieve-nexus-analog) + ENGINEERING §9 + gap `http_submit_poll_workspace` 同步 | `test_api_runs.py` 覆盖 submit/list/summary/repro；矩阵 **partial** + caveat「无厂商配额」 |
| **5** | B5 Qermit MitRes/MitEx | 否 | **开放 DAG + 线性迹 + `mitigation_execution_model`** 与 [errmit 手册](https://docs.quantinuum.com/inquanto/manual/errmit.html) 公开节对表；MitEx 批量 → SLA | [mitigation_PMSV_ZNE_Qermit_mapping.md](/concept/mitigation-mapping) 与 `capability-surface` 同源；ZNE/PMSV 键完整 | gap `qermit_graph` status 与单测；**不宣称**商业 Qermit |
| **6** | B6 CuTensorNet | 否 | **stub + 引擎解析 + `tensornet_engine_resolved`**；化学尺度收缩 → SLA 或 L3 | [附录 B §7](/parity/gap-implementation-plan#l3-benchmark-suite-roadmap) 挂钩；矩阵 TN 行与 `tensornet/` 一致 | pytest stub + 可选引擎探测；gap `tensornet` |
...
| **10** | C4 VQD | `run_summary` + `three_protocol`（已部分落地） | [工程记忆 §3.1](/concept/engineering-memory-quantinuum) + 矩阵 | `test_orchestration_pipeline` + extended 单测 |
...
| **16** | D2 DMET 框架 | Schmidt 密度反馈 + `cycles_executed` / 钩子 | [技术文档_DMET与parity_snapshot开放契约.md](/reference/dmet-parity-snapshot) + gap `dmet_scf_loop` | Schmidt 生产单测 |
...
| 序 | 对应分项 | L1 目标 | 交付物 | 验收（DoD） |
|----|----------|---------|--------|-------------|
| **20** | E1–E3 TKET 链 | `CompilerSpec` + **tket_first 探针**与 [技术文档_CircuitIR与TKET桥接及作业契约.md](/reference/circuitir-tket-jobs) §2–4 **一致**；Ion 专有 routing → **n/a 叙事** | gap `compiler_pass_bundle`；可选默认 `pytket` CI job | `test_tket_compiler_narrative` + `test_pytket_bridge` |
...
| 序 | 对应分项 | L1 目标 | 交付物 | 验收（DoD） |
|----|----------|---------|--------|-------------|
| **21** | F1 Computable 薄层 | `ComputableSpec` 或与 `ComputableRef` **双向转换**（L1 签字稿 [附录 C](/parity/gap-implementation-plan#appendix-c) 原三周日历口径）；**不**承诺闭源融合顺序 | `protocols/computable.py` + workflow-preview 可选 `computables_rich` | 单测 + 矩阵 **`composable_computable` 状态升级** |
...
| 序 | 对应分项 | L1 目标 | 交付物 | 验收（DoD） |
|----|----------|---------|--------|-------------|
| **23** | G1 三能量路径 + DataFrame 叙事 | 在 [技术文档_设备比特串与Qiskit采样路径.md](/reference/qiskit-shot-counts) 与 export 中 **对齐「云侧 shot 叙事」的开放等价物**（非真云） | 矩阵 Protocol 行收束 caveat | `classify_pauli_expectation_path` + parity_export CI |
...
| **25** | I1 MD/ML 长板 | **签认差异化** | [竞争定位](/concept/competitive-positioning) + README | `md_bridge` smoke（若有）或文档签认 |
...
- 进度钉扎：[附录 B](/parity/gap-implementation-plan#appendix-b) §3 度量表每月一行；**序 1–26** 完成度映射到「无 unexplained `partial`」。
- 闭源不可达项：一律在 [附录 B §6](/parity/gap-implementation-plan#y1-residual-partial-sla-template) 落表。
...
**钉扎日期**：2026-05-07（审计生成时仓库状态）。**范围**：竞争定位 §6「P1 开源工作流超越」四行 + 差距文档 §3 摘要表中 **P1 中程** 三条（证据见本文 §2）+ [附录 C](/parity/gap-implementation-plan#appendix-c) 主表与 H1/I1 + 与 P1 叙事强相关的 parity 主表（§1–§3）及 [附录 D](/parity/gap-implementation-plan#appendix-d) 交叉引用。**不含**：路线图 P2/P3 实现（见 [附录 A](/parity/gap-implementation-plan#appendix-a)）、真 Nexus/H 系、L0 闭源等价。
...
*版本：正文 §1–§6 为差距与维护入口；**附录 A–F** 收编原 P2 / Y1 / L1 / B→J / P1 审计 / 不排期项独立稿。与 [inquanto_public_parity_matrix.md](/parity/public-matrix) 表同源维护。*
