...
- **开放栈「记忆 + 缺口清单」**：[记忆_开放栈对标完成度与待闭合项.md](/parity/open-stack-memory)（闭源不可见部分的自研边界）
...
- **上述能力维护记忆**：[记忆_HTTP_API与作业队列_工程记忆.md](/concept/http-api-worker-memory)
...
- **本地 FastAPI 类比**（提交/列表/轮询、`parity-gaps`、`computables-preview`、`queue-stats`、`repro`-only）：[技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](/reference/http-api-sqlite-jobs)；维护清单见 [记忆_HTTP_API与作业队列_工程记忆.md](/concept/http-api-worker-memory)。
...
| **HTTP / SQLite 队列 / 可观测性** | [技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](/reference/http-api-sqlite-jobs)；[记忆_HTTP_API与作业队列_工程记忆.md](/concept/http-api-worker-memory) |
...
*文档版本：与仓库源码同步维护；重大行为变更时请更新 §1（缓解行）、§4、§7、§9、**§11** 及 [技术文档_CircuitIR与TKET桥接及作业契约.md](/reference/circuitir-tket-jobs)；必要时 bump 配置 `schema_version`（若引入破坏性 YAML 字段）。*
