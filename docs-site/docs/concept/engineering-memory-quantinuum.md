...
- **HTTP 维护决策与 Checklist**：[技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](/reference/http-api-sqlite-jobs) **§9**（原 `记忆_HTTP_API与作业队列_工程记忆.md` 已合并）
...
4. **文档**：本文 + `inquanto_contract.inquanto_gap_categories()` 同步更新口径；**维护用记忆与缺口清单**见 [工程记忆_Quantinuum对标与数据流技术文档.md](/concept/engineering-memory-quantinuum) **§13**；**DMET/`parity_snapshot` 字段契约**见 [技术文档_DMET与parity_snapshot开放契约.md](/reference/dmet-parity-snapshot)。
...
- **缓解图 + 线性执行**：`mitigation_graph_report`（`qermit_analog`）与管线输出的 `mitigation_dag_execution`（`qermit_runtime`）。详见 [与InQuanto能力差距与实施计划 — 附录 F](/parity/gap-implementation-plan)、[inquanto_public_parity_matrix.md](/parity/public-matrix)。
...
原「按对标可证伪性排序」的逐项 checklist 已由 **[与InQuanto能力差距与实施计划 — 附录 E/C](/parity/gap-implementation-plan)** 与正文 **§3 摘要表**收束；**勿在本节追加新 checkbox**。物化链、判据与缺口仍以 **§1–§8**、**§13** 为准；路线图 **P2** 见 [与InQuanto能力差距与实施计划 — 附录 A](/parity/gap-implementation-plan)。
...
可选 **pytket** 资源双轨、`README` 的 parity 入口表、`JobHandle.protocol_hash` 与 `retrieve` 语义已落地。**维护入口**：[技术文档_CircuitIR与TKET桥接及作业契约.md](/reference/circuitir-tket-jobs)（自研 `spec` vs 可选 pytket）、`backends/pytket_bridge.py`、`README.md`、`jobs/store.py`、[launch_retrieve_nexus_analog.md](/concept/launch-retrieve-nexus-analog)。自研 `depth`/`twoq` 与 `pytket_*` 列的论文写法见该技术文档 **§3**。更广的「开放栈 gap 闭合」见 **§13**。
...
**用途**：把对方 **用户指南级** 叙事（算法 ↔ 可计算量 ↔ 协议 ↔ pytket 后端）映射到本仓库**可检证**的模块与 JSON 出口。**不**声称与 InQuanto 闭源包或默认工作流二进制一致；边界见 [竞争定位与路线图_对标Quantinuum产品与技术路线.md](/concept/competitive-positioning) §4。
...
|--------------|---------------------|-------------|
| `Algorithm*` 求解化学量（基态/激发等） | `qchem_stack/quantum/algorithms/`（VQE、ADAPT、IQEB、VQD/QSE/SCEOM 等）；激发态 `run_summary` 见 [工程记忆 §3.1](/concept/engineering-memory-quantinuum) | `repro.parity_snapshot` 量子段；`export_parity_criteria_table.py` |
| 符号层 **Computable** + **Protocol** 评估 | `protocols/computable.py`、`protocols/protocol.py`（`PauliAveragingProtocol` 五阶段）；**图预览** `integrations/inquanto_workflow_preview.py` | `POST /v1/meta/workflow-preview`、`POST /v1/meta/computables-preview`；[parity 矩阵 §1](/parity/public-matrix) |
| **pytket** 驱动编译与后端 | 可选：`backends/pytket_bridge.py`、`integrations/tket_fullchain.py`；资源/门集叙事见 [技术文档_CircuitIR与TKET桥接及作业契约.md](/reference/circuitir-tket-jobs) | `parity_snapshot.tket_first_compiled_circuit_probe`（若启用） |
...
| FCIDUMP 互操作 | 以 PySCF 为主路径；FCIDUMP **未**作为一等入口时在 [parity 矩阵 §3](/parity/public-matrix) 标保守 `partial` | |
| Embedding（DMET 等手册分支） | `chem/embedding/`、`integrations/schmidt_dmet_self_consistent.py` 等 | [技术文档_DMET与parity_snapshot开放契约.md](/reference/dmet-parity-snapshot) |
...
| 公开叙事要点 | `qchem_stack` 落点 |
|--------------|---------------------|
| Protocol 上挂缓解 | `mitigation/`、`MitigationSpec`；叙事对照 [mitigation_PMSV_ZNE_Qermit_mapping.md](/concept/mitigation-mapping) |
| 资源估计 | `backends/spec.py`：`dataframe_circuit_shot_rows` 等 |
| 作业队列（产品向「云 UX」） | **本地类比**：`qchem_stack.api`、`jobs/`；见 [launch_retrieve_nexus_analog.md](/concept/launch-retrieve-nexus-analog) |
...
- 公开站结构改版时：对照 [howto](https://docs.quantinuum.com/inquanto/manual/howto.html) 侧边栏是否重排；更新本页 **章节标题对齐** 与 [与InQuanto — 附录 C](/parity/gap-implementation-plan) 钉扎说明。
- 单一真相顺序不变：**代码键** → `export` / `repro` → [parity 矩阵](/parity/public-matrix) → `inquanto_gap_categories()`。

---

*版本：初版；与 Y1 台账 [与InQuanto — 附录 B](/parity/gap-implementation-plan) 的「公开文档周一对照」一致。*
...
- 能力差距与边界：[与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan)、本文 **§0**
...
- Y1 台账与 [与InQuanto — 附录 B](/parity/gap-implementation-plan) 引用；
- 镜像页 `status`（shipped / partial / placeholder / not-applicable）的**二次校验**；
- 论文 Methods 中「开放栈做了什么 / 未声称什么」的表述依据。

### 1.2 不声称的范围（L0 排除）

本对照 **不** 声称与 **闭源 `inquanto-pyscf` wheel**、**InQuanto 内部默认启发式** 或 **类名级 API 一一对应**（L0）。对齐口径为公开资料可追溯 + 本仓可跑路径 + `repro` / parity 机读键（L1），见 [与InQuanto — 附录 D](/parity/gap-implementation-plan)。
...
| 纠偏项 | 权威依据 | 维护动作 |
|--------|----------|----------|
| 镜像 `status` vs 正文 | 本文 §2–§4、`open_driver_coverage_matrix` | 改版镜像 YAML 时对照 [与InQuanto — 附录 D](/parity/gap-implementation-plan) **附录 A** |
...
| DMET 概览 | `manual` / embedding | partial | `chem/embedding/dmet.py` + 管线 DMET 钩子 | **中** | `DMETContext`、占位 solver；字段契约见 [技术文档_DMET与parity_snapshot开放契约.md](/reference/dmet-parity-snapshot)；口径与 [inquanto_public_parity_matrix.md](/parity/public-matrix) §3 一致 |
...
|-----------------|---------------------|----------|----------|----------|
| **AVAS** | shipped（部分镜像页） | **中**（PySCF：`active_space.strategy=avas` + `qchem_stack.chem.active_space.pyscf_active_space_hooks` → PySCF `mcscf.AVAS`） | **无** InQuanto 同名 `qchem_module` **类绑定**；开放栈通过配置 + 管线钩子实现 **阈值 AVAS**（`driver_meta.qchem_active_space_resolution_v1`，样例 `configs/example_h2_avas.yaml`） | 矩阵 / gap：**相对产品**仍为 **partial**（非「无 AVAS」一刀切）；镜像若宣称 shipped，应注明 **等价物为 PySCF 路径**，避免与源码事实冲突 |
...
- 镜像页 `status` 或 `qchem_module` 变更时：同步检查本文 §4 对应行，并更新 [与InQuanto能力差距与实施计划.md](/parity/gap-implementation-plan) §1 经典化学行（若涉及公开承诺）。
...
**母稿**：[与InQuanto能力差距与实施计划 — 附录 A](/parity/gap-implementation-plan) §6 序 5、§8 第 1–2 周；SLA 行见 [附录 B §6 — `y1-residual-partial-sla-template`](/parity/gap-implementation-plan#y1-residual-partial-sla-template)。
...
*文档版本：与仓库源码同步维护；重大行为变更时请更新 §1（缓解行）、§4、§7、**§11**、**§13** 及 [技术文档_CircuitIR与TKET桥接及作业契约.md](/reference/circuitir-tket-jobs)；**§9** 仅保留归档指针。必要时 bump 配置 `schema_version`（若引入破坏性 YAML 字段）。*
