...
- **自研**（源码 `qchem_stack/backends/spec.py`）：`_depth_estimate` 按门在 qubit 上的层推进；`_twoq_gate_count` 统计名称属于集合 `CX|CNOT|ZZ|ZZPhase|MS|CP` 等。`ANNOTATION` 若带 `qubits` 可能仍影响层数（取决于实现细节）；**与 pytket 不完全同构**。
...
- **`retrieve(store, handle)`**：等同 `SqliteJobStore.result(handle.job_id)`。`status != DONE` 时**不得**假定存在 `expectation`；详见 [launch_retrieve_nexus_analog.md](/concept/launch-retrieve-nexus-analog)。
