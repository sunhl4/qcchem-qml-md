...
| GET | `/health/ready` | 无 `schema`；体为 `{"status":"ready","job_db_default":...}`；失败 **503** |
...
| POST | `/v1/runs` | 同步 **200**：`full_pipeline_job_result_v1`（`JSONResponse`，带 trace 头）；异步 **202**：`run_enqueue_response_v1` |
