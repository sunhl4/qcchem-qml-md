...
5. **分解插件玩具**：**`configs/example_decomposition_plugin_toy.yaml`**（`embedding.mode: plugin`）与矩阵 §3「分解插件」行及 [公开 Parity 矩阵](/parity/public-matrix) 对照。

## 收束

把每次运行的 **`experiment_id` / `random_seed`** 与 **`repro`** 片段存档，便于与 CI 或 [Parity](/parity/public-matrix) 导出对齐（若你负责验收）。
