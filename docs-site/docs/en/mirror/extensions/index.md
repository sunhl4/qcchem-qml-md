---
title: Extensions
reference_doc_url: "https://docs.quantinuum.com/inquanto/extensions/extensions-overview.html"
diataxis: reference
pillar: meta
status: partial
qchem_module: ""
milestone: ""
i18n_key: extensions
---

# Extensions <StatusBadge :status="$frontmatter.status" />

<p class="mirror-breadcrumb">Manual mirror node path: <code>extensions</code> · <a href="/mirror/extensions/">中文版</a></p>

::: info Mirror metadata
- **Status**: Partial
- **Pillar**: Meta (navigation)
- **Diátaxis**: Reference
- **Module**: *(placeholder, not yet implemented — see milestone)*
- **Milestone**: —
- **Reference doc URL**: [https://docs.quantinuum.com/inquanto/extensions/extensions-overview.html](https://docs.quantinuum.com/inquanto/extensions/extensions-overview.html)
:::

## What it is

This page mirrors a third-party public documentation entry. Use the reference anchor for upstream wording; see "Our implementation" for this repository.

## Our implementation

**Partial** — Module: *(placeholder, not yet implemented — see milestone)*

Fields or behavior may differ from the public reference text; see [Capabilities & roadmap](/en/product/roadmap).

## Children in this branch

<MirrorBranch :prefix='["extensions"]' :grouped='true' locale="en" />


## Related

- [Capabilities & roadmap](/en/product/roadmap)
- [Engineering architecture](/en/concept/engineering-architecture)
- [Product roadmap](/en/product/roadmap)
- [15-minute quickstart](/en/tutorial/quickstart)
- [IA slug map](/en/meta/ia-mapping)
