...
Machine-readable summary (same as HTTP): **`capability_surface.open_stack_differentiators`** (schema **`open_stack_differentiators_v1`**: `scope_excludes`, `beyond_public_doc_bundle`) plus **`operator_pool_registry_export_v1`** in the same payload (ADAPT/IQEB pool ids and **`pool_id_aliases`**).
...
- **Criteria export:** `scripts/export_parity_criteria_table.py` (YAML + optional results JSON).
