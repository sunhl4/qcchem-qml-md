...
Multi-fragment sweeps are implemented via :meth:`~qchem_stack.chem.embedding.dmet_self_consistent.DMETSelfConsistencyLoop.run_with_sequential_bath_updates`.
"""
...
def _initial_ao_density_state(
    rhf: ClassicalMeanFieldReference,
) -> tuple[np.ndarray, np.ndarray, int]:
    ao = rhf.ao_basis_view()
    S = ao.overlap_ao()
    D = ao.make_rdm1_ao()
    nel = int(round(float(np.trace(S @ D))))
    return S, D, nel
...
def run_schmidt_density_feedback_cycles(
    ...
    *,
    ...
    """
    rhf_ref = _schmidt_feedback_reference(rhf, context="schmidt density feedback cycles")
    if max_cycles < 1:
        raise SchmidtProductionError("max_cycles must be >= 1")
    S, D, nel = _initial_ao_density_state(rhf_ref)
...
def run_schmidt_multifragment_density_cycles(
    ...
    **Multi-fragment** Gauss–Seidel on global ``D``, implemented with
    ...
    """
    rhf_ref = _schmidt_feedback_reference(rhf, context="schmidt multifragment density cycles")
