"""Unitary Coupled Cluster with Generalized Doubles (UCCGD) — JW/BK square encodings only."""

from __future__ import annotations

from typing import TYPE_CHECKING

from qchem_stack.chem.kernels.spin_ucc import build_spin_uccgd_fermion_generators
...
class UCCGDVQE(UCCSDVQE):
    """Generalized-doubles cluster ansatz (superset of spin-UCCSD paired doubles on small spaces)."""
    ...
    def __init__(
        ...
        *,
        ...
        if mapping == "symmetry_conserving_bravyi_kitaev":
            raise ValueError(
                "UCCGD dense cluster ansatz requires a square fermion encoding "
                "(jordan_wigner or bravyi_kitaev with n_spin_orbitals == n_qubits)."
            )

        fs = hamiltonian.fermion_space
        if fs is None:
            raise ValueError("UCCGDVQE requires hamiltonian.fermion_space for electron count.")
        self._fermion_mapping = mapping
        self._n_so = int(fs.n_spin_orbitals)
        self._n_e = int(fs.n_electrons)
        if self._n_so != self.n_qubits:
            raise ValueError(
                f"JW/BK-square UCCGD expects n_spin_orbitals == n_qubits ({self._n_so} vs {self.n_qubits})."
            )

        ferm_ops = build_spin_uccgd_fermion_generators(self._n_so, self._n_e)
    ...
    def _run_meta_base(self) -> dict[str, object]:
        ...
        meta["variational_ansatz"] = "uccgd"
        meta["uccgd_n_parameters"] = self.n_params
        return meta


def uccgd_algorithm_report_v1(result: UCCSDVQEResult) -> dict[str, object]:
    from qchem_stack.contracts.schema_ids import ALGORITHM_UCCSD_REPORT_V1

    return {
        "schema": ALGORITHM_UCCSD_REPORT_V1,
        "algorithm": "vqe",
        "variational_ansatz": "uccgd",
