...
def build_spin_uccsd_fermion_generators(
    ...
    *,
    ...
    Singles couple one occupied index ``i`` to one virtual index ``a``; doubles couple two
    occupied and two virtual indices. Anti-Hermitian cluster operators for VQE follow from
    subtracting the Hermitian conjugate in fermion space (then JW), in downstream code.
    """
    occ = list(range(n_electrons))
    virt = list(range(n_electrons, n_spin_orbitals))
    ops: list[FermionOperator] = []
