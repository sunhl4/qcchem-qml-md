    ...
    if cfg.embedding.mode == "projection":
        ...
        if emb.projection_quantum_hamiltonian == "fragment_mulliken_mo":
            ...
            trace["caveat"] = (
                ...
                "PySCF CASCI active integrals, and JW (see projection_hamiltonian)."
            )
            ...
            trace["caveat"] = (
                "Variational stage uses the global :class:`ActiveSpaceSpec` JW Hamiltonian "
                "(same as ``embedding.mode: none`` for the qubit operator). "
                "This block records projection workflow metadata only."
            )
            trace["epistemic_bound"] = (
...
def _run_dmet_fragment_solve_if_requested(
    ...
    if cfg.embedding.dmet_hamiltonian_source != "whole_active_system":
        return
    labels = [x for x in cfg.embedding.fragment_labels if str(x).strip()]
    if len(labels) != 1:
    ...
    ledger = OneShotEmbeddingDriver.run(ctx, {labels[0]: qh})
...
def _schmidt_hamiltonian_and_context(
    ...
    audit: dict[str, Any] = {
        ...
        "note": (
            "Main variational stage uses impurity JW Hamiltonian; cfg.active_space sizes apply to "
...
def _attach_qpe_demo_track_if_requested(
    ...
    if not cfg.quantum.qpe_demo_track_after_variational:
        return
    from qchem_stack.qpe_qec_demo import BayesianQPEStub, kitaev_qpe_energy_estimate

    out["qpe_demo_track"] = {
        "schema": "qpe_qec_demo_track_v1",
        "kitaev_ground_energy_dense": float(kitaev_qpe_energy_estimate(qh, bits=4)),
        "bayesian_phase_map_toy": BayesianQPEStub().estimate([(0.0, 0.5), (1.0, 1.0)]),
    }
...
def _attach_run_summary(out: dict[str, Any], cfg: ExperimentConfig) -> None:
    ...
    if isinstance(qse_out, dict):
        ...
        if isinstance(qmeta, dict) and qmeta.get("shot_noise_model"):
    ...
    if isinstance(sceom_out, dict):
        ...
        if isinstance(sceom_meta, dict) and sceom_meta.get("shot_noise_model") is not None:
...
def run_pipeline_sync(
    ...
    *,
    ...
    qh, schmidt_ctx = _hamiltonian_with_schmidt_context(cfg, rhf)
    ...
    if cfg.embedding.mode == "dmet":
        ...
        if cfg.embedding.dmet_hamiltonian_source == "whole_active_system":
            wf["impurity_solver_used"] = (
                "qchem_stack.chem.embedding.dmet.QubitHamiltonianFragmentSolverVQE"
            )
...
def run_pipeline_from_config(
    ...
    *,
    ...
    proto = _protocol_for_job(cfg, qh, angles)
