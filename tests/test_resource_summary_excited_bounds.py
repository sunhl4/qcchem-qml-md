...
from qchem_stack.orchestration.pipeline import (
    _excited_shot_channel_upper_bounds,
    _excited_shots_upper_bound,
    _resource_summary_excited_only,
)
...
def test_excited_shots_upper_bound_qse_pauli_tasks_times_shots_fallback() -> None:
    ...
    assert _excited_shots_upper_bound(ex) == 320
...
def test_excited_shots_upper_bound_qse_prefers_schedule_over_task_fallback() -> None:
    ...
    assert _excited_shots_upper_bound(ex) == 100
...
def test_resource_summary_excited_only_shape() -> None:
    ...
    rs = _resource_summary_excited_only(2, er)
...
def test_excited_shot_channel_upper_bounds_table() -> None:
    ...
    b = _excited_shot_channel_upper_bounds(ex)
    assert b["vqd"] == 8 + 4
    assert b["qse"] == 50
    assert b["sceom"] == 0
