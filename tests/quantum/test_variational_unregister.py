...
def test_unregister_removes_registration_and_keeps_builtin_intact() -> None:
    ...
    builtins_before = frozenset(BUILTIN_VARIATIONAL_PLUGIN_IDS)
    assert is_registered_variational_id(pid)
    unregister_variational_plugin(pid)
    assert not is_registered_variational_id(pid)
    assert frozenset(BUILTIN_VARIATIONAL_PLUGIN_IDS) == builtins_before
    for bid in BUILTIN_VARIATIONAL_PLUGIN_IDS:
        assert is_registered_variational_id(bid)
