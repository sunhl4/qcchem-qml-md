---
title: InQuanto 公开契约 vs qchem_stack 覆盖矩阵
description: 公开叙事可对齐能力的覆盖状态矩阵；与仓库 docs/inquanto_public_parity_matrix.md 同源维护。
keywords:
  - InQuanto
  - parity
  - capability matrix
...
> **权威母稿**：仓库根目录 **`docs/inquanto_public_parity_matrix.md`**（与 `docusaurus-site/` 同级）。本站页仅为 **原则与导航**，全文矩阵以母稿为单一事实源。
>
> **中英路由**：当前文档站以中文树为主；未来若启用 Docusaurus `i18n`，请在英文镜像页顶部保留 **同一母稿路径** 链接（避免矩阵双稿漂移）。

本页用于记录“公开叙事可对齐能力”的覆盖状态：`yes / partial / no / n/a`。

**维护**：本站正文与仓库根 [**`docs/inquanto_public_parity_matrix.md`**](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/inquanto_public_parity_matrix.md) **同源更新**（PR 请两处一并修改）。

**目的**：仅对照 [Quantinuum 公开文档与 API](https://docs.quantinuum.com/inquanto/)，说明本仓库 **独立实现** 的等价程度；**不**声称复现闭源二进制或默认超参数。

**用户指南级叙事映射（How to use）**：[工程记忆 §14](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/工程记忆_Quantinuum对标与数据流技术文档.md)（How-to 映射）对照官方 [How to use InQuanto](https://docs.quantinuum.com/inquanto/manual/howto.html) 与本仓库模块 / `repro` 出口。

**逐项闭合执行计划（B→J 顺序，L1 对拍定义）**：[与 InQuanto 能力差距与实施计划 — 附录 D](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/与InQuanto能力差距与实施计划.md#appendix-d)

**差距清单 + 分阶段计划（维护稿）**：[与 InQuanto 能力差距与实施计划.md](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/与InQuanto能力差距与实施计划.md)（机读分类：`qchem_stack.protocols.inquanto_contract.inquanto_gap_categories`）。**L1 签字清单**：[附录 C](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/与InQuanto能力差距与实施计划.md#appendix-c)。

**边界（必读）**：把「公开叙事 + 可检证」做到底是 **L1**；**完全复现 InQuanto 闭源包在内部默认值、全部 driver 组合与商业运行时细节**属于 **L0**，在无源码与许可下**不可作为工程承诺**。见 [工程记忆 §0](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/工程记忆_Quantinuum对标与数据流技术文档.md)。

**图例**：`yes` 已实现 / `partial` 部分或 API 形态不同 / `no` 未实现 / `n/a` 刻意不做或非公开范围

## 0. 非云、非专有硬件：对齐范围与「超越」（L1+）

**全面对齐（本仓承诺）**：除下述 **刻意排除** 项外，矩阵 §1–4 与机读表 `inquanto_gap_categories()`、`GET /v1/meta/capability-surface` **同源**；`partial` 均配有 gap 锚点、caveat 或 [Y1 SLA 模板（附录 B §6）](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/与InQuanto能力差距与实施计划.md#y1-residual-partial-sla-template)。

**执行状态（2026Q2）**：Day12–Day90 连续执行已完成本轮收口（`docs/execution/day90_final_closeout_2026Q2.md`）；**Tangelo 差距日历滚动收口**见 `docs/execution/day090_tangelo_calendar_closeout.md`（与 `docs/execution/day001_day090_tangelo_gap_calendar.md` 同源）。后续迭代计划见 `docs/execution/day91_next_phase_plan_2026Q3.md`（Day91–Day120 日拆解：`docs/execution/day91_day120_daily_breakdown_2026Q3.md`）。**2026Q3 追加 60 天**：InQuanto 矩阵 × Tangelo 广度 × 机读 HTTP 再对照 — `docs/execution/day121_day180_inquanto_tangelo_calendar_2026Q3.md`（Kickoff：`docs/execution/day121_kickoff_inquanto_tangelo_reconciliation_2026Q3.md`；封板：`docs/execution/day180_signoff_inquanto_tangelo_2026Q3.md`）。
...
|----------|----------|-------------|
| 五阶段 instantiate→build→compile→run→evaluate | [Protocols overview](https://docs.quantinuum.com/inquanto/manual/protocols_overview.html) | `partial`：五阶段有等价；可选 `run_sampled`（statevector MC）与 `run_qiskit_shots_pauli_protocol`（Qiskit `get_counts` / Aer/硬件，见 [设备比特串与 Qiskit 采样路径](../reference/qiskit-shot-counts)）；五阶段上可挂 **`nexus_analog` 计价**与 **`zne_scales`**（若启用 ZNE）。异步侧 **非** Nexus 1:1，见 [launch/retrieve 对照](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/launch_retrieve_nexus_analog.md)（`JobHandle` 含 `protocol_hash`，本地 SQLite 队列 + worker；pickle 协议上带 `NexusAnalogSpec` 与同步计价一致） |
| `dataframe_circuit_shot` 式资源表 | [Resource estimation](https://docs.quantinuum.com/inquanto/manual/protocols/resource_estimation.html) | `yes`：`dataframe_circuit_shot_rows` + `spec.dataframe_circuit_shot`；导出侧可选 `resource_estimation_preview_v1`（`parity_integrations.resource_estimation_preview: true`）用于 Methods 轻量切片，**非**云计价、非闭源 L0 resource estimator。并可在同开关下导出 `algorithm_registry_alignment_v1` / `md_ml_repro_freeze_fields_v1` 作为 W5/W6 机读证据块（内含 **`operator_pool_registry_export_v1`**）。 |
...
| 作业提交 / 列表 / 轮询（产品网关） | Nexus / 云侧 UX 叙事 | `partial`：**本地 FastAPI 类比** `qchem_stack.api`：`POST/GET /v1/runs`、`GET /v1/meta/parity-gaps`、**`POST /v1/meta/computables-preview`**、**`GET /v1/meta/queue-stats`**；无厂商身份与配额，见 [工程架构](../concept/engineering-architecture) §9、[launch/retrieve 对照](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/launch_retrieve_nexus_analog.md) |
...
| Qermit `MitRes`/`MitEx` | [Noise mitigation](https://docs.quantinuum.com/inquanto/manual/errmit.html) | `partial`：`PMSV`/`ZNE`/`SPAM` 存根；可选 **`mitigation.classical_shadows_stub_enabled`**（DAG：`classical_shadows_expectation_stub` 置于 SPAM 之后、PMSV 之前；`configs/example_h2_classical_shadows_stub.yaml`；迹Homology见 `tests/test_mitigation_dag_trace_homology.py`）；可选 **`mitigation.zne_mode=circuit_scale_fold`**（每尺度 HEA 放大 + `protocol_counts.zne_curve` 写入 **`mitigation_dag_execution`**）；**ZNE×Qiskit Pauli** 与 YAML 口径合一机读块 **`parity_snapshot.zne_qiskit_unification_v1`**（见 [mitigation 与 Qermit 映射](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/mitigation_PMSV_ZNE_Qermit_mapping.md)）；Methods：`resource_estimation_preview_v1` / `methods_resource_unified_v1` 镜像 **`mitigation_zne_mode_yaml`**、**`mitigation_zne_scales_yaml`**，`--results` 另附 **`parity_snapshot_mitigation_zne_*`**；**机读图** `mitigation/qermit_analog`（`qermit_analog_v2`：`nodes`+`edges`+`topological_order`）；**线性执行迹** `mitigation/qermit_runtime` → `mitigation_dag_execution`（非 Qermit 商业运行时）；**L1 顺序不变量**：DAG 中 SPAM/PMSV/ZNE 节点 `kind` 序列与执行迹 `mitigation_dag_execution` 中每项的 `node` 字段一致（shadows stub 纳入同一不变量测试） |
...
| `AlgorithmSCEOM` | `partial`：`run_sceom_nested_commutator`（D2SC05371C 型嵌套交换子矩阵元 M<sub>ij</sub>，Pauli 玩具生成元）+ 参考子空间路径 |
...
| `AlgorithmBayesianQPE` + Phayes | `partial`：`qpe_qec_demo/bayesian_stub.py`（`BayesianQPEStub`）；模块说明见 [`qpe_qec_demo/README.md`](https://github.com/sunhl4/qcchem-qml-md/blob/main/src/qchem_stack/qpe_qec_demo/README.md)；并入 `qpe_demo_track` / `run_summary.qpe_demo_track_ran`；同上 **`run_summary.qpe_open_stack_contract_v1`**；单测 `tests/test_l1_phase_c_iqeb_bayesian.py` |
...
| InQuanto 全量 driver/方法名表（COSMO、PBC、多 k…） | `partial`：名称映射击 `chem/inquanto_driver_surface`；**PySCF** 上已实现 **ddCOSMO**、**PBC**（`pbc_kpoint_mesh`：Γ 为 `RHF`，否则 `KRHF`）、**PBC+ddCOSMO 尝试**（受 PySCF 版本约束）；`chemistry_extended.rdm_correction_method=stub_*|pyscf_nevpt2_casci`（stub：零校正机读；`pyscf_nevpt2_casci`：**PySCF `mrpt.NEVPT`** on CASCI，开放钩子 **非** InQuanto L0）；**非** 与 InQuanto 闭源 `inquanto-pyscf` 行级一一覆盖 |

**Parity export CI 抽样（几何 / SCF 扩展，config-only）**：`configs/example_h2_sto3g_density_fit.yaml`、`example_h2_zmatrix_sto3g.yaml`、`example_h2_zmatrix_sto3g_density_fit.yaml`、`example_mg_lanl2dz_ecp_rhf.yaml`、`example_mg_lanl2dz_ecp_density_fit.yaml`、`example_hbr_zmatrix_lanl2dz_ecp_density_fit.yaml` — 与本页 **§3**、`scripts/check_parity_export_sample.py` 内 `SAMPLE_CONFIGS_REL` **一致**（另含 Psi4 浅路径样例 `configs/example_h2_psi4_rhf_sto3g.yaml` 等，见脚本清单）。**`geometry_source`** 在上述导出中可被 CI 的稳定键门禁覆盖（见 `inquanto_contract.PARITY_EXPORT_V2_STABLE_KEYS`）。
...
- **编译 / TKET（`compiler_pass_bundle`）**：**默认**管线以 `CompilerSpec` + 自研 `CircuitIR` 为主，**不**要求安装 TKET；**可选** `pytket` 且开启 `parity_integrations.tket_first_circuit_stats` 时，才写入 `parity_snapshot.tket_first_compiled_circuit_probe`（Pauli 协议已编译出 `CircuitIR` 时）。Ion 阱专有 routing 与 InQuanto 私有 pass 包 **不对齐**。叙事与字段见 [CircuitIR 与 TKET 桥接及作业契约](../reference/circuitir-tket-jobs) §2–4。
- **可复现**：YAML、`protocol_hash`、job 元数据、版本写入 `orchestration` 流水线；`JobHandle.protocol_hash` 与 SQLite `jobs` 表对账，详见 [CircuitIR 与 TKET 桥接及作业契约](../reference/circuitir-tket-jobs) §6。
...
- **判据表导出**：`scripts/export_parity_criteria_table.py`（YAML + 可选运行结果 JSON）；config-only 顶键 **`geometry_source`**（取值 `cartesian` 或 `zmatrix`），与 **`molecular_system_from_experiment`** / `MoleculeSpec` 同源，便于 Methods 与竞品「问题构造」表对读。
...
**竞争定位与路线图**母稿：[`竞争定位与路线图_对标Quantinuum产品与技术路线.md`](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/竞争定位与路线图_对标Quantinuum产品与技术路线.md)。站内摘要：[竞争定位与路线图](../concept/competitive-positioning)。
