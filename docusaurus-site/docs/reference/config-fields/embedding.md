---
title: embedding 配置字段
description: embedding.mode（none/dmet/projection/plugin）与 Schmidt、投影字段。
---
...
**详细说明：** [说明_embedding配置.md](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/说明_embedding配置.md)
...
| `dmet_max_cycles` | DMET 外循环（$\le$ `SCHMIDT_DMET_MAX_CYCLES_LIMIT`） |
...
把**已有分片 Pauli 哈密顿量**（JSON）注入管线，不走完整 DMET / 投影构造。

| 字段 | 说明 |
|------|------|
| `plugin.name` | 插件名（内置：`uniform_fragment_guess`） |
| `plugin.json_path` | 分片载荷 JSON 路径 |

**怎么用：** [分解插件教程](/tutorial/decomposition-plugin-minimal)。  
**谁在用：** pre-quantum 建哈密顿量、embedding_workflow 审计字段。

相关：[嵌入总览](/modules/chem/embedding) · [Pre-quantum 矩阵](/reference/pre-quantum-yaml-matrix)。
