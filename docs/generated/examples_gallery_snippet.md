...
> **Source of truth:** repository [`examples/README.md`](https://github.com/sunhl4/qcchem-qml-md/blob/main/examples/README.md). Regenerate with `python scripts/generate_examples_gallery.py`.

## Examples index（仓库根 `examples/`） / Examples index

与 [CONTRIBUTING.md](https://github.com/sunhl4/qcchem-qml-md/blob/main/CONTRIBUTING.md) 中的 CI、`scripts/check_parity_export_sample.py` 及 **Docusaurus**（`docusaurus-site/`）教程并列使用。
...
| **张量网络 / DMRG toy（自旋链）** | `toy_dmrg_spin_chain.py`；配套讲义 [`docs/tensor_network_qchem_self_study.md`](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/tensor_network_qchem_self_study.md) |

详细叙事见站点 [onboarding-three-paths](/guide/onboarding-three-paths)（`cd docusaurus-site && npm start`）。
...
- 设计说明：[`docs/solver_entrypoint_plugin_安装与发布指南.md`](https://github.com/sunhl4/qcchem-qml-md/blob/main/docs/solver_entrypoint_plugin_安装与发布指南.md)
...
- 指南：[化学与嵌入](/guide/chemistry-and-embedding)（站内 `/guide/chemistry-and-embedding`）
