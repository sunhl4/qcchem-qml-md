...
InQuanto 的设计理念是**「真机与运营一体」**——强类型 API 把 **`Computable` / `Protocol`、TKET 深度编译优化、Nexus（队列·配额·计费）与 H-Series 离子阱后端**串成同一套产品链，几行代码就能把哈密顿量推到**可运维、可对账的云上真实测量**，省去自建「化学栈 + 编译器 + 作业系统」的零散拼接：
...
在经典层已按 §4.2 产出统一结构（`PySCFRHFResult`，或经 `embedding.mode == "plugin"` 等路径注入的等价哈密顿量数据）之后，下文将 SCF 参考转为 **`QubitHamiltonian`**——**自此与「经典程序品牌」解耦**。
...
**配置文件（出图专用）**：`configs/example_h2_vqe_figure_near_casci.yaml`（与 `configs/example_h2.yaml` 几何/活性空间一致；**UCCSD** + `use_pauli_protocol: false`）

![VQE Convergence](assets/vqe_convergence_demo.png)

- CASCI 参考：$-1.137276$ Ha（PySCF `CASCI` 总能量；此活性空间下与 FCI 一致，图中红色虚线）
- 纵轴曲线：**UCCSD** 簇展开幅在 **$|\theta_k|\le 0.38$** rad 盒约束下，用 **scipy L-BFGS-B** 记录每次 `<ψ(θ)|H|ψ(θ)>`；初值 **$\theta=0$**（对应 HF 参考态）。数据由 `docs/assets/export_vqe_convergence_trace.py` 写入 `docs/assets/data/vqe_h2_sto3g_jw_near_casci_trace.json`
- 设计动机：同一 qubit 哈密顿量上，**无界 COBYLA/L-BFGS-B** 常落入约 **−1.192 Ha** 的浅极小（与 CASCI 不一致）；**小盒 + L-BFGS-B** 将轨迹约束在**变分上界**一侧，使末态可落在 **≈1 mHa** 量级接近 CASCI（见 JSON 内 `note`）

> 图注：真实优化轨迹（非示意）；参考线为 PySCF CASCI。出图配置与默认 `example_h2.yaml`（HEA+Pauli）分离，避免混读。

---

### 5.2 经典方法 vs 量子方法能量对比
...
平台在 `active_space.fermion_qubit_mapping` 支持 **Jordan–Wigner**（`jordan_wigner`）、**Bravyi–Kitaev**（`bravyi_kitaev`）、**对称守恒 BK / SCBK**（`symmetry_conserving_bravyi_kitaev`）。映射的详细原理、实跑资源对比与工程取舍见：**[费米子量子比特映射_JW_BK_SCBK_详细分析.md](费米子量子比特映射_JW_BK_SCBK_详细分析.md)**。
