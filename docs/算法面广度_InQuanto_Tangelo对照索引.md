...
| VQD / QSE / SCEOM | `quantum/algorithms/excited.py`, `quantum/qse_transition.py`, `quantum/algorithms/sceom.py`，契约：`excited_*_bundle_v1`、`excited_protocol_contract_v1`；**VQD 行为与 YAML**：[技术文档_VQD紧缩激发与跨栈对照.md](技术文档_VQD紧缩激发与跨栈对照.md) |
...
- 设置 **`QCHEM_RUN_L3=1`**：`pytest -m l3`，对 **`integrations.l3_algorithm_benchmark.L3_PYTEST_YAMLS`** 管线回归（默认 **7** 条：基线 **`example_h2.yaml`（VQE）** + ADAPT singles/doubles、`uccsd_jw` **别名**、IQEB fermionic doubles、`qubit_excitation` **别名**、excited-smoke）；也可用 `scripts/l3_algorithm_benchmark_report.py`（默认 **`DEFAULT_BENCHMARK_YAMLS`**）导出 **`algorithm_benchmark_bundle_v1`**（能量、`nfev`、耗时等）做论文/对标表；`--merged` 附带 **`merged_experiment_benchmark_v1`**（总量 + **`by_quantum_algorithm_yaml`** 分组）。
- Roadmap：`docs/execution/day91_next_phase_plan_2026Q3.md` Week 5–6；**60 天再对照日历**（矩阵 × 本索引 × capability-surface）：`docs/execution/day121_day180_inquanto_tangelo_calendar_2026Q3.md`（起算见 `day121_kickoff_inquanto_tangelo_reconciliation_2026Q3.md`）。

## 5. 本仓「广 / 深」收口声明（2026Q3）

- **完整深度路线图（母文档）**：[工程深度路线图_InQuanto_Tangelo_对标与交付.md](../工程深度路线图_InQuanto_Tangelo_对标与交付.md)（能力轴 × 工作包 × 闸门 × 非目标；与本节互补）。
