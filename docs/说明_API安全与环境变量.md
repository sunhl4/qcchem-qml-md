...
- **未设置**：不挂载 `AuthenticationMiddleware`，本地开发可直接访问。
- **已设置**：请求须带 `Authorization: Bearer <QCHEM_STACK_API_KEY>`，否则返回 `401`/`403`。
...
`PauliAveragingProtocol.dumps()` / job enqueue 使用 **HMAC 签名的 pickle**（32 字节签名 + payload）。
