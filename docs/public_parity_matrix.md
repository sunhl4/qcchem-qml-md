...
| DMET 框架 / fragment solver 钩子 | `partial`→**available（声明适用体系）**：`chem/embedding/dmet.py` + **`dmet_self_consistent.py` PySCF 密度反馈 bath v1**（H₄/H₂ dimer demo）；Schmidt 侧车、projection 编排；**`n/a`**：cuTensorNet 化学尺度 |
...
用户可读产品定位摘要：[docusaurus-site/docs/product/positioning.md](../docusaurus-site/docs/product/positioning.md)（站点路由 `/product/positioning`）。
