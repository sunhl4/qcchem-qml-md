...
| Embedding       | DMET 量子在 fragment                            | `chem/embedding/dmet.py`；`embedding.dmet.hamiltonian_source`（`parity_stub` / `whole_active_system` 单片段真 VQE）；`repro.parity_snapshot` 中 `dmet_*`；`integrations/dmet_self_consistent` 自洽骨架                                                               | 与竞品 §4 一致：论文需写清自洽轮数与经典基线档；**多片段 bath DMET 仍依赖用户钩子**                                                                                                                              |
