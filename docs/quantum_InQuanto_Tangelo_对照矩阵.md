...
- Tangelo 全套 BuiltInAnsatz：**executable**：HEA、UCCSD、**UCCGD**、**QCC**、**UpCCGSD**、**pUCCD**；**partial 别名**：VSQS→HEA；**外环开放算法**：iQCC/iQCC+PT（`algorithm: iqcc`）；**研究插件**：QITE；**`n/a`**：JKMN/HCB 映射（见 `fermion_mapping_registry`）
