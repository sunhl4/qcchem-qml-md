...
```bash
./scripts/venv-run pytest tests/chem/test_pre_quantum_path.py tests/chem/test_build_pre_quantum_input_api.py -q
```
...
| **`excited_plugins/registry.py`** | 激发态 sidecar（VQD / QSE / SCEOM）插件调度；`excited_registry_export_v1` 与 variational export 对称 | `orchestration/excited_stages.py` 调用 |
| **`variational_branch.py`** | UCCSD vs HEA 共享分支（VQE 基态 + VQD `prepare_state`） | `builtins` 与 `excited_plugins/builtins` |
| **`algorithms/vqe.py`** | 最常见的：**调电路参数让能量尽量低** | 用 `backends` 里的执行器算期望值；pipeline 输出 `algorithm_report`（`algorithm_vqe_report_v1`） |
| **`algorithms/adapt.py`、`iqeb.py`** | 逐步加算符、加深电路的算法 | 用 `operator_pool_registry.py` 里的「候选算符池」；`algorithm_report` 经 `AlgorithmBase.generate_report()` |
| **`algorithms/uccsd_vqe.py`** + **`algorithms/uccsd_mapping.py`** + **`algorithms/uccsd_circuit.py`** | UCCSD 变分 ansatz、JW/BK 映射与 CircuitIR prep | `variational_branch`；`uccsd_algorithm_report_v1`；JW 下可选 `pauli.use_protocol: true`（见 `configs/example_h2_uccsd_pauli_protocol.yaml`） |
| **`algorithms/excited*.py`** | 激发态算法类（VQD、QSE、SCEOM） | 经 `excited_plugins/builtins` 接线；资源估算见 orchestration `excited_resource_summary` |
| **`algorithms/__init__.py`** | 子包 lazy export（`__getattr__`） | 按需加载算法类，避免 eager import |
| **`statevector.py`** | HEA statevector 与 dense Pauli 期望 | 采样协议、激发态 HEA 路径 |

**Handoff 约定：** 变分与激发态均通过 `VariationalRunContext` / `ExcitedRunContext` 的 `pre_quantum_input` + `resolved_hamiltonian()` 优先 canonical Hamiltonian；orchestration 在 `run_excited_stages` 转发 `pre_q_input`。
...
| **`protocols/ansatz_prep.py`** | Ansatz × Protocol：HEA / UCCSD prep 与 Pauli 测量电路解耦 |
...
```bash
# 用项目约定的 Python 跑测试
./scripts/venv-run pytest tests/chem/test_pre_quantum_path.py -q
