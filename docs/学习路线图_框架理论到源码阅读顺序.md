...
|--------|--------------|-------------------|
| **`config/`** | 读 YAML、变成程序里的「设定对象」、检查设定是否矛盾 | 改设定、加新开关时 |
...
3. 再进入化学层的统一入口 `classical_mean_field_via_solver_bridge`（`chem/bridges/facade.py`）：根据 YAML 里的 `scf.driver` 选求解器（`chem/solvers/registry.py` → PySCF 的 `drivers/pyscf_driver.py`）。
4. PySCF 算完后，结果被包成 **`ClassicalMeanFieldReference`**（`chem/bridges/mean_field_reference.py`）——对上游来说，**不管底层是 PySCF 还是 Psi4，长得都一样**。

**这一步留下什么**

- **总能量** `e_tot`（氢分子在这个基组下的 RHF 能量，单位原子单位 au）。
- **PySCF 的计算对象**（内部还要用轨道去抽积分；量子阶段不会直接碰它）。
- **`driver_meta`**：备注，例如「这次是 PySCF 算的」「活性空间策略是 manual」等。
...
**你可以跟别人这样说**  
「`pre_quantum_path` 像路口指示牌；`builder_registry` 像『每条路派哪个施工队』。加新路时登记一个新施工队就行，不用改总调度的大逻辑。」
...
| 插件分解 | `embedding.mode: plugin` | 能量公式主要来自外部文件，不现场从 SCF 推 |
| Schmidt 生产 | 嵌入里开 Schmidt 生产 | 先做大分子嵌入，在「杂质区」建哈密顿量，常依赖 PySCF 内部对象 |
| 投影片段 | 投影 + 指定 Mulliken 分子轨道 | 在分子**片段**上取活性空间，不是全分子默认取法 |
...
| `chem/drivers/pyscf_driver.py` | 真的调 PySCF |
...
| **`molecule.py` 等** | 原子坐标、基组、电荷 | 建分子时用 |
...
| **`drivers/pyscf_driver.py` 等** | 具体调 PySCF / Psi4 |
...
| **`pre_quantum_pyscf_gate.py`** | 有的路必须用 PySCF 内部对象，否则清楚报错 |
...
| **`variational_plugins/builtins.py`** | 把名字接到具体算法文件 | 调用 `algorithms/` |
...
| **`algorithms/uccsd_vqe.py`** | 另一种簇展开风格的变分 | 和简单 VQE 不同套路 |
| **`algorithms/excited.py`** | 激发态（例如 VQD、QSE） | 流水线里「基态算完」之后再调 |
| **`statevector/hea_state.py`** | 把一维参数变成具体量子电路 | 采样协议、激发态都会用到同一套角度布局 |
...
| **`excited_stages.py`** | 激发态阶段的顺序调用 | 算激发态时 |
