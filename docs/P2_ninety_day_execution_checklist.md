...
**作用**：对照用户保存的 **「90 天对标商业闭源栈 / 借鉴 Tangelo」** Cursor 计划原文（本地 `.cursor/plans/90-day_stack_comparison_plan_*.plan.md`）**每一天**，标明状态与仓库内证据。  
**母稿 parity**：仍以仓库根 [`public_parity_matrix.md`](public_parity_matrix.md) 为准；Docusaurus 节选页 [`docusaurus-site/docs/parity/public-matrix.md`](../docusaurus-site/docs/parity/public-matrix.md) 顶部说明了母稿路径。
...
| 日 | 计划摘要 | 状态 | 证据 / 备注 |
|----|-----------|------|-------------|
| D1 | Vendor platform How-to ↔ [`Vendor platform_manual_howto_与_qchem_stack_映射.md`](Vendor platform_manual_howto_与_qchem_stack_映射.md) 不一致清单 | Done | [`P2_execution_alignment_notes.md`](P2_execution_alignment_notes.md) §1 drift 表（2026-05-12 基线行）；钉扎见 [`L1_Vendor platform_alignment_signoff.md`](L1_Vendor platform_alignment_signoff.md) |
...
| D4 | 矩阵 ↔ `product_gap_categories()` | Done | **`protocols/product_contract.py`**、`scripts/check_parity_export_sample.py`、[`tests/api/test_api_runs.py`](../tests/api/test_api_runs.py)；维护约定见 [CONTRIBUTING.md](../CONTRIBUTING.md#product-contracts-and-workflow-preview-stable-imports)（历史 `tests/api/test_api_runs.py` 已移除） |
| D5 | export 抽样字段齐全 | Done | `tests/repro/test_export_parity_golden.py`、`export_parity_criteria_table.py` |
...
| D17 | Computable ↔ workflow-preview | Done | `tests/repro/test_workflow_preview_repro_alignment.py`、`protocols/computable.py` |
| D18 | HTTP capability / workflow-preview ↔ contract | Done | `tests/api/test_api_runs.py`（需 `[api]`） |
...
| D22 | 缓解键 ↔ errmit 映射审计 | Done | [`mitigation_PMSV_ZNE_Qermit_mapping.md`](mitigation_PMSV_ZNE_Qermit_mapping.md)「MitigationSpec YAML 键」；`tests/mitigation/test_mitigation_output_schema.py` |
| D23 | `parity_snapshot` 语义可追溯 | Done | 编排/`docs/技术文档_DMET…`/`export_parity_criteria_table`；历史中心化 frozenset 单测已移除 |
...
| D28 | embedding_config 回归 | Done | `tests/repro/test_repro_snapshot.py`；代表：`example_h2_embedding_parity.yaml` |
| D29 | Y1 台账 | Done | [`Vendor platform_Y1_public_alignment_ledger.md`](Vendor platform_Y1_public_alignment_ledger.md) |
...
| 日 | 计划摘要 | 状态 | 证据 / 备注 |
|----|-----------|------|-------------|
| D31–D33 | 分解 demo / embedding / 单测 | Done | `configs/example_decomposition_plugin_toy.yaml`、`tests/chem/test_decomposition_plugin_pipeline.py` |
...
| D43 | 激发态 YAML export | Done | `pytest -m l1_excited`；`tests/repro/test_export_parity_schema.py::test_excited_export_config_only_for_vqd_yaml`；`configs/example_h2_excited_smoke.yaml` |
| D44 | gap_closure_bundle | Done | `integrations/gap_closure_bundle.py`；`tests/quantum/test_gap_closure_bundle_minimal.py` |
| D45 | **闸门 C** | Done | 同上 CI |
...
| D47 | 矩阵 §1–§3 备注 | Done | 矩阵 §0「同源」叙事 + **`product_gap_categories()`** § 锚点；`scripts/check_parity_export_sample.py` |
...
| D52–D55 | MD/ML smoke repro | Done | `tests/md_bridge/test_md_bridge.py`、`tests/integrations/test_ml_surrogate_l1_md_ml.py`、`md_bridge_repro_freeze_list.md` |
| D56 | L3 路线图窗口 | Done | [`L3_benchmark_suite_roadmap.md`](L3_benchmark_suite_roadmap.md) 文末「90 日批次」 |
| D57 | health/ready / SQLite 契约 | Done | [`技术文档_HTTP_API与SQLite作业队列及可观测性契约.md`](技术文档_HTTP_API与SQLite作业队列及可观测性契约.md) §5；`tests/api/test_api_runs.py` |
...
| D66–D67 | B→J-21 computable round-trip | Done | `tests/quantum/test_computable_roundtrip_minimal.py` |
...
| D80 | computable_graph_v2 压力测试 | Done | [`tests/protocols/test_workflow_preview_contract.py`](../tests/protocols/test_workflow_preview_contract.py)（`computable_graph_v2` 套件，如 `test_computable_graph_v2_roundtrip_refs_re_emit_identical`） |
| D81 | workflow-preview fuzz | Done | `tests/api/test_api_runs.py` |
| D82 | **闸门 G** | Done | CI |
...
| D89 | L1 signoff 更新 | Done | [`L1_Vendor platform_alignment_signoff.md`](L1_Vendor platform_alignment_signoff.md) § `ninety_day_wave` |
