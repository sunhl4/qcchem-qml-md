...
| D4 | 矩阵 ↔ `product_gap_categories()` | Done | **`protocols/product_contract.py`**、`scripts/check_parity_export_sample.py`、[`tests/test_api_runs.py`](../tests/test_api_runs.py)；维护约定见 [CONTRIBUTING.md](../CONTRIBUTING.md#product-contracts-and-workflow-preview-stable-imports)（历史 `tests/test_gap_parity_matrix_anchors.py` 已移除） |
...
| D80 | computable_graph_v2 压力测试 | Done | [`tests/test_workflow_preview_contract.py`](../tests/test_workflow_preview_contract.py)（`computable_graph_v2` 套件，如 `test_computable_graph_v2_roundtrip_refs_re_emit_identical`） |
