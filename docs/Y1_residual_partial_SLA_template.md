...
| `tensornet` | TN 化学尺度收缩 | **n/a**（开放栈 stub；不宣称 `inquanto-cutensornet`） | Y2-Q2 | 见 MAINTAINERS | 见矩阵 §1 |
| `drivers_cosmo_pbc` | 全 driver 表面 / 多 k / 溶剂边界 | partial_kmesh | Y1-Q4 | 见 MAINTAINERS | PySCF 版本；变更 `PYSCF_MIN_VERSION_RECOMMENDED` 时同步矩阵 §3 + `test_inquanto_driver_surface_l1` |
...
| **矩阵 §2 `AlgorithmAdaptVQE`** | pool / 日程与 InQuanto/Tangelo 公开「化学激发池」非逐条对齐 | partial + 文档对照节（parity §2 下「ADAPT 与公开 pool」） | Y2-Q1 | 见 MAINTAINERS | `tutorial_inquanto_chain_h2.yaml`；`adapt.py` |
