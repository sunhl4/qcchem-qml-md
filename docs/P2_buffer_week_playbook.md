...
- 对齐 [`public_parity_matrix.md`](public_parity_matrix.md) 与 **`product_gap_categories()`**（HTTP `gaps` / export `capability_gap_categories`；历史 `test_gap_parity_matrix_anchors.py` 已移除，闸改由 `tests/api/test_api_runs.py` 等与 CI 脚本承担）。
