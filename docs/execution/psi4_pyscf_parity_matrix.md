...
|------|----------------|----------------------------------------|
| `tests/chem/test_psi4_pyscf_h2_canonical_parity.py` | `constant` 5e-3; `h1` max abs 5e-2; `h2` max abs 8e-2 | `psi4_casci_hamiltonian_v1` or `psi4_mints_casci_effective_v1` (see `driver_meta.kernel_bindings`) |
| `tests/chem/test_psi4_pyscf_alignment.py` | SCF energy, MO overlap, embedding smoke | `psi4_energy_scf_v1` vs `pyscf_native_v1` |

Run: `pytest -m psi4 tests/chem/test_psi4_pyscf_h2_canonical_parity.py tests/chem/test_psi4_pyscf_alignment.py`
