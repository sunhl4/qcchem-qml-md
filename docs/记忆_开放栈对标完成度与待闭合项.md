...
**文档性质**：给维护者与未来自己的**决策记忆**（非产品对外白皮书）。口径与 [架构_InQuanto闭源能力闭合与可复现边界.md](架构_InQuanto闭源能力闭合与可复现边界.md) 一致：追求 **L1 公开契约可审计**，不追求 **L0 闭源二进制等价**。

**关联技术说明**：

- [技术文档_DMET与parity_snapshot开放契约.md](技术文档_DMET与parity_snapshot开放契约.md)（DMET 字段、single-fragment 演示、YAML）
- [工程记忆_Quantinuum对标与数据流技术文档.md](工程记忆_Quantinuum对标与数据流技术文档.md)（模块级数据流）
- [技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](技术文档_HTTP_API与SQLite作业队列及可观测性契约.md)（可选 FastAPI、`SqliteJobStore`、`run_context` / `pipeline_profile`）
- [记忆_HTTP_API与作业队列_工程记忆.md](记忆_HTTP_API与作业队列_工程记忆.md)（上述层的维护清单与非目标）
...
| Nexus / HQC 工作流 | `nexus_public_workflow_blueprint`、既有 `nexus_cloud` / `nexus_analog` / `qnexus_probe`；**本地**可选 HTTP + `SqliteJobStore`（`api/app.py`，见 [技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](技术文档_HTTP_API与SQLite作业队列及可观测性契约.md)） | **无** 真计费/队列二进制；HTTP 仅为类比网关，非对方生产 API |
...
- 能力变化时同步：`inquanto_gap_categories()`、[inquanto_public_parity_matrix.md](inquanto_public_parity_matrix.md)（若行级矩阵变更）。
- 新增 `parity_snapshot` 顶层键时：更新本文 §2、[技术文档_DMET与parity_snapshot开放契约.md](技术文档_DMET与parity_snapshot开放契约.md)（若 DMET 相关）。
- 变更 **HTTP 路由** / **作业表 `meta`** / **`run_context` 头** / **`pipeline_profile`** 时：同步 [技术文档_HTTP_API与SQLite作业队列及可观测性契约.md](技术文档_HTTP_API与SQLite作业队列及可观测性契约.md) 与 [记忆_HTTP_API与作业队列_工程记忆.md](记忆_HTTP_API与作业队列_工程记忆.md)，并跑 `tests/test_api_runs.py`、`tests/test_job_store_list.py` 等。
