...
**文档角色**：在 [`Quantinuum_竞争判断与对标建议.md`](../../PandM/materials/learning/quantum-chem/literature/Quantinuum_竞争判断与对标建议.md) 与 [公开 parity 矩阵](inquanto_public_parity_matrix.md) 之上，**规定本仓库（`qchem-stack`）作为「可竞争产品」的边界、差异化与分阶段工程目标**。**不**声称在离子阱真机、InQuanto 闭源或 Nexus 商业云上「功能对等」。
...
**一句话**：以 **InQuanto + 自家硬件** 为交付单元，**PySCF + TKET + Nexus** 为公开可见的产品三角。用户指南级工作流总览（算法 / computable / protocol / pytket）见官方 [How to use InQuanto](https://docs.quantinuum.com/inquanto/manual/howto.html)；与本开源栈的可检证映射见 [InQuanto_manual_howto_与_qchem_stack_映射.md](InQuanto_manual_howto_与_qchem_stack_映射.md)。
...
2. **公开「可证伪判据」出口**：`parity_snapshot`、`export_parity_criteria_table`、Hamiltonian `meta`、资源双轨（`spec` + 可选 [pytket](技术文档_CircuitIR与TKET桥接及作业契约.md)）。  
...
6. **HTTP 机读「非云超越钉扎」**：`GET /v1/meta/capability-surface` 的 **`open_stack_differentiators`**（`open_stack_differentiators_v1`）列出在**排除商业云与专有硬件标定**前提下、相对公开文档包的可检证长板；与 [parity 矩阵 §0](inquanto_public_parity_matrix.md) 一致。
...
- 追求与 **Qermit 商业运行时、inquanto-cutensornet、InQuanto 闭源 driver 目录** 的 **1:1** 替代（本仓库已提供 **开放栈可对表类比**：`qermit_analog`+`qermit_runtime`、`tensornet` stub+引擎钩子、PySCF 上 **ddCOSMO/PBC（含 k 网）** 等，见 [不排期项_转排期与实现说明.md](不排期项_转排期与实现说明.md) / [parity 矩阵](inquanto_public_parity_matrix.md)——**仍非** 产品与闭源等价）。  
...
|------|----------|----------|---------------|
| **P0** | 判据与可复现**叙事闭合** | 完善 `export_parity_criteria_table` 与 `repro` 在 CI 的固定例；任何 pipeline 大改时更新 [parity 矩阵](inquanto_public_parity_matrix.md) | `scripts/`、`orchestration/`、§2 工程记忆 |
...
| **P2** | 多后端**设备真 shot**（可选） | `PauliAveragingProtocol` 的 `run_qiskit_shots_pauli_protocol` + `backends/qiskit_pauli_shots.py`：Aer/IBM 等经 Qiskit `Backend.run` 的 `get_counts` 与 `resource_rows` 对齐；`HamiltonianExpectationExecutor` 仍用于非 shot 路径的精确期望 | [技术文档_设备比特串与Qiskit采样路径.md](技术文档_设备比特串与Qiskit采样路径.md) |
| **持续** | **MD/ML** 与化学核**契约稳定** | `md_bridge` schema、QMEF 与上游 `repro` 字段一致 | `md_bridge/` |

**已落地（工程执行）**：P0 已加 `export_parity_criteria_table` 扩展字段、CI 中 **parity 导出 + `--sampled` + `run_qpe_track_demo`**；P0 `run_sampled` 见 `configs/example_h2_sampled.yaml`；P1 已加 **`EmbeddingSpec` + `repro`、PMSV YAML + `pmsv_report`、`excited_shot_accounting`**；P2 已加 **`scripts/run_qpe_track_demo.py` + `configs/qpe_dual_track_demo.yaml`**、`pauli_measurement_ledger`；P2 已加 **Qiskit 比特串 Pauli 路径**（`run_qiskit_shots_pauli_protocol`、`configs/example_h2_qiskit_shots.yaml`、见 [技术文档_设备比特串与Qiskit采样路径.md](技术文档_设备比特串与Qiskit采样路径.md)）；持续项已加 **`QMFrame.repro_config_sha256_prefix`**。**另（开放栈可对表）**：`nexus_analog`/`nexus_cloud` 侧车、`qermit_*`、`tensornet` 钩子、PBC+k 点/ddCOSMO 等已进主包，见 [README](../README.md) 与 [工程记忆](工程记忆_Quantinuum对标与数据流技术文档.md)。**仍为后续**：Quantinuum 专有云真机一键编排、主 pipeline 内 QPE 与 FT 深度合流、ZNE 与多倍真实电路行严格对齐。

---

## 6. 与现有「工程记忆 / parity」的衔接

- 详细差距与实现顺序见 [工程记忆_Quantinuum对标与数据流技术文档.md](工程记忆_Quantinuum对标与数据流技术文档.md)（激发态、Protocol `run`、shots、ADAPT 元数据等）。  
- 公开能力覆盖见 [inquanto_public_parity_matrix.md](inquanto_public_parity_matrix.md)。  
- **能力差距 + 排期**（与 InQuanto 公开栈逐项对照、含 `pauli_protocol_expectation_path`）：[与InQuanto能力差距与实施计划.md](与InQuanto能力差距与实施计划.md)。  
