...
公开 InQuanto 文档根页以 **Chemical Specification / Program Construction / Execution and Analysis** 三支柱组织产品心智；How-to 入口见官方 [How to use InQuanto](https://docs.quantinuum.com/inquanto/manual/howto.html)，本仓映射见 [工程记忆 §14](工程记忆_Quantinuum对标与数据流技术文档.md)。
...
| 分子输入              | InQuanto-PySCF、geometry、express、active space | PySCF/Psi4/预计算积分                                           | **PySCF** 为现阶段端到端数值主线；**Psi4 等**经 `scf.driver` + solver registry **占位扩展**（见 **§5.1**）；统一 interchange 见 `ClassicalMeanFieldReference` | 支持 PySCF；其它程序按 §5.1 契约增量接入   |
...
**已闭合批次（不在本战略文铺陈证据链）**：P0/P1、主线结构增强（QPE 演示轨/主 pipeline、Computable 薄层、TKET CI）及原「不排期」四项之 **开放栈类比 v1**，均已以审计与 CI 收口。权威清单见 [与InQuanto能力差距与实施计划 — 附录 E / F](与InQuanto能力差距与实施计划.md)；机读键与矩阵见 [inquanto_public_parity_matrix.md](inquanto_public_parity_matrix.md)、[与InQuanto能力差距与实施计划.md](与InQuanto能力差距与实施计划.md) §1–§2；**物化链与判据**见 [工程记忆_Quantinuum对标与数据流技术文档.md](工程记忆_Quantinuum对标与数据流技术文档.md)。

**仍需推进（§141 残余，多为路线图 P2）**：QPE/FT **资源估计与编译器级深度合一**（浅层演示轨已交付，**深度**见 [与InQuanto能力差距与实施计划 — 附录 A](与InQuanto能力差距与实施计划.md) 内 P2-W1）；**化学意义上**完整 DMET bath 自洽与 **产品级** ONIOM/QM-MM；**InQuanto/Vendor 默认捆绑的 AVAS+CASSCF「产品全流程」叙事**（开放栈已对 PySCF **阈值 AVAS**、`casscf_orbital_optimization_*` **单次核**接线，矩阵仍 honest **`partial`**）；**BK/SCBK 上 UCCSD Trotter**（矩阵 **`n/a`**）；**教程与 examples** 资产扩张。**分阶段 backlog** 见 [与InQuanto能力差距与实施计划.md](与InQuanto能力差距与实施计划.md) §6；**WBS / 闸门** 见同文 **附录 A**。代表性 YAML 与 `test_backend_conformance` 见 [README](../README.md)。
...
| **P2→P3 过渡（Q3/Q4）** | 教程与 examples 对新用户闭环                     | 完善 docs-site 索引与三条上手路径，持续双站同源                            | 新用户按文档可跑通最小链路 |
...
- B-J 闭合顺序与 L1 验收哲学见 [与InQuanto能力差距与实施计划 — 附录 D / C](与InQuanto能力差距与实施计划.md)。
...
- **三块母稿分工**：战略与三方对比（**本文**）；物化链 / 判据 / 模块索引（[工程记忆](工程记忆_Quantinuum对标与数据流技术文档.md)）；差距表 + 维护约定 + **附录 A–F**（[与 InQuanto 能力差距与实施计划](与InQuanto能力差距与实施计划.md)）。路线图 **P2** 分解见同文 **附录 A**。
